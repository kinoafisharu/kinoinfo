...
@only_superuser
@never_cache
def edit_api_dev(request):
    from api.forms import ApiDevForm
    fileName = getApiDevFileName(request)
    if request.POST:
        form = ApiDevForm(request.POST)
        if form.is_valid():
            content = request.POST['text'].encode('utf-8')
            f = open(fileName,'w')
            f.write(content)
        ...
        f = open(fileName,'r')
        content = f.read()
    form = ApiDevForm(initial={'text': content},)
    f.close()
    return render_to_response('api/edit_dev.html', {'form': form}, context_instance=RequestContext(request))
