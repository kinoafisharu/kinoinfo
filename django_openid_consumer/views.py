...
def begin(request, redirect_to=None, on_failure=None, template_name='user/login.html', lj_url=None):
...
def complete(request, on_success=None, on_failure=None, failure_template='user/login.html'):
    ...
    query_dict = dict([
        (k, v) for k, v in request.GET.items() # (k.encode('utf8'), v.encode('utf8')) for k, v in request.GET.items()
    ])
    ...
    if openid_response.status == SUCCESS:
        ...
        return on_failure(request, _(u'Авторизация была отменена'), failure_template)
...
def default_on_success(request, identity_url, openid_response):
    ...
    if not next or not is_valid_next_url(request, next):
        next = getattr(settings, 'OPENID_REDIRECT_NEXT', '/user/login/openid/auth/')
    
    return HttpResponseRedirect(next)

def default_on_failure(request, message, template_name='user/login.html'):
    #return render(template_name, {
    #    'message': message
    #}, context_instance=RequestContext(request))
    return render('user/login.html', {'message': message}, context_instance=RequestContext(request))
