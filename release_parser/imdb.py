            ...
            for li in ul.findAll('li'):
                ...
                if 'documentary' not in low(year):
                    ...
                    if year:
                        ...
                        if 'limited' not in low(details) and 'fest' not in low(details) or 'tv premiere' not in low(
                                details):
                            ...
                            xml += '<film n="%s" s="%s" y="%s" id="%s" d="%s" r="%s"></film>' % (
                            film_name, film_slug, year[0], imdb_id, details, rel_date)
                            ids.append(imdb_id)
...
def parse_imdb(main_data, count, source, imdb, is_dump, images, country_data, genres_data, persons_data, productions,
               distr_objs, film_object, films, language, distr_nof_data, data_nof_persons, nof_distr, nof_persons,
               release_format, country_id, release):
    limits = {
        'G': 0,
        'PG': 6,
        'PG-13': 12,
            ...
            for distrib in distributors:
                if distrib['year'] == cur_year:
                    ...
                    if distr_obj:
                        ...
                        distrib['name'].replace('&', '&amp;'), distr_slug, None)
        ...
        for pe in persons:
            ...
            if person_obj:
                ...
                if person_id not in nof_persons:
                    ...
                    data_nof_persons += '<person name="%s" slug="%s" code="%s" name_alt="" slug_alt=""></person>' % (
                    person_name, person_slug, person_id)
...
def create_film_by_imdb_id(imdb):
    ...
    try:
        ...
        count, film_obj, distr_nof_data, data_nof_persons, nof_distr, nof_persons = parse_imdb(main_data, count, source,
                                                                                               imdb, is_dump, images,
                                                                                               country_data,
                                                                                               genres_data,
                                                                                               persons_data,
                                                                                               productions, distr_objs,
                                                                                               film_object, films,
                                                                                               language, distr_nof_data,
                                                                                               data_nof_persons,
...
def get_imdb_data(xml, is_dump, country_id=1, ids=[], upd=False, film_kid=None):
    ...
    for i in xml.findAll('film'):
        ...
        if parse:
            count, film_obj, distr_nof_data, data_nof_persons, nof_distr, nof_persons = parse_imdb(main_data, count,
                                                                                                   source, imdb,
                                                                                                   is_dump, images,
                                                                                                   country_data,
                                                                                                   genres_data,
...
def imdb_film_ident():
    ...
    for i in films:
        ...
        if kid:
            ...
            data_nof_film += xml_noffilm(name, slug, None, None, i.imdb_id, 'Фильм не найден', full_url.encode('utf-8'),
                                         source.id)
...
def imdb_film_show(request):
    ...
    for films in filmsdata:
        ...
        txt += '<h2><a href="http://www.imdb.com/title/tt%s/">%s (%s)</a></h2>' % (
        films.imdb_id, name.name.encode('utf-8'), films.year)
        ...
        if table:
            ...
            for tr in table.findAll('tr'):
                ...
                if go and year:
                    ...
                    result.append(
                        {'title': title, 'persons': persons, 'link': link, 'year': year, 'aka': aka, 'id': imdb_id})
