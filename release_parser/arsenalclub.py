...
def get_arsenalclub_schedules():
    ...
    for d in range(delta.days + 1):
        ...
        flag = False
        url = '%skino/?rasdel=kino&day=%s#daybox' % (source.url, date.strftime('%d.%m'))

        req = urllib.urlopen(url)
        if req.getcode() == 200:
            data = BeautifulSoup(req.read()) #, from_encoding="utf-8"
            
            for table in data.findAll('table', width="100%", cellpadding="3", cellspacing="1", bgcolor="#393939"):
                ...
                if len(trs) == 0:
                    ...
                    for tr in trs:
                        ...
                        full_url = film.a.get('href').encode('utf-8') if film.a and film.a.get('href') else None
                        ...
                        if full_url:
                            film_id = full_url.replace('http://www.kinopoisk.ru/film/','').encode('utf-8')
                        else:
                            film_id = film_slug
                        ...
                        if film_id not in noffilms and film_slug.decode('utf-8') not in ignored:
                            ...
                            if next_step:
                                ...
                                if kid:
                                    ...
                                    data_nof_film += xml_noffilm(film_name, film_slug, None, None, film_id, info, full_url, source.id)
...
@timer
def arsenalclub_schedules_export_to_kinoafisha():
